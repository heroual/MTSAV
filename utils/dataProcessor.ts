  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Read as array of arrays to handle the row 3 headers and row 4 data
        ...
        const tickets: Ticket[] = dataRows
          .filter(row => row[idxND]) // Filter empty rows
          .map((row, index) => {
            ...
            // Règle spécifique : Si le groupe est VOIP FTTH, on force le produit à VOIP
            if (groupe.includes('VOIP FTTH')) {
              produitRaw = 'VOIP';
            }

            // Regroupement VOIP et VOIP FO
            if (produitRaw.toUpperCase() === 'VOIP FO') {
              produitRaw = 'VOIP';
            }
            
            const motifRaw = String(row[idxMotif] || 'Inconnu');
            // Traduction des abréviations en texte clair
